...
### 9.5 중복 클라이언트 실행 문제 (2025-06-22 해결됨)
- **증상**: 같은 컴퓨터에서 여러 클라이언트 실행 시 기존 클라이언트 연결 끊김
- **해결**: 프로세스 기반 중복 실행 방지 기능이 구현됨
- **확인**: 클라이언트 실행 시 "⚠️ 다른 클라이언트 프로세스 발견" 메시지가 나타나면 정상 동작
- **문서**: `01_3_v2.0_duplicate_client_prevention_solution.md` 참조

### 9.6 psutil 라이브러리 오류
- **원인**: 중복 실행 방지 기능에 필요한 psutil 라이브러리가 설치되지 않음
- **해결**: 
```bash
cd client
pip install psutil==5.9.5
```
- **확인**: requirements.txt에 psutil==5.9.5가 포함되어 있는지 확인
